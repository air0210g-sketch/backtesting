...
def get_credentials(config_path):
    ...
    with open(config_path, 'r') as f:
        try:
            data = json.load(f)
            # Traverse to find the env vars. Check mcpServers first.
            env = {}
            if "mcpServers" in data and "longport-mcp" in data["mcpServers"]:
                 ...
                 if "env" in data["longport-mcp"]:
                     env = data["longport-mcp"]["env"]
                 else:
                     env = data["longport-mcp"] # direct keys
...
def main():
    ...
    # Updated default to relative path from scripts/
    default_output = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "stock_data")
    parser.add_argument("--output-dir", default=default_output, help=f"Directory to save CSV. Default: {default_output}")
